...
const scrapeDynamicWebpage = () => {
    ...
    await page.goto('https://www.telemart.pk/mobile-and-tablets/mobile-phone/apple-iphones.html');
...
function processData(data) {
    ...
    listItems.each(function (idx, ele) {
      ...
      const item = {id: idx + 1, name: name, link: link, img: img, price: price}
      items.push(item);
    });
    // Logs items array to the console
    // console.dir(items);
    // Write items array in itemsData.json file
    fs.writeFile(`${dataDir}itemsData.json`, JSON.stringify(items, null, 2), (err) => {
      if (err) {
        console.error(err);
        return;
      }
